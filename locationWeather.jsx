...
function WeatherDisplay({weatherData}){
    ...
    const {temp, description, icon, windTransform, windSpeed} = React.useMemo(() => {
        ...
        return {
            ...
            windSpeed: weatherData ? Math.round(weatherData.wind.speed) : 0,
        }
    }, [weatherData]);

    return(
        <>
            {temp && <Typography variant="h6"></Typography>}
            {icon && (
...
function locationWeather({location}) {
    ...
    React.useState(() => {
        const LoadingIndicator =setTimeout(() => setIsLoading(true), 500);
        const getWeather = async () => {
            const result = await getLocationWeather(location);
            clearTimeout(LoadingIndicatorTimeout);
            setIsLoading(false);
            setWeatherData(result.success ? "" : result.error);
    ...
    const { flagIcon, contryCode } = React.useMemo(() => {
        return {
    ...
    return (
        <>
            <div className={className.headerLine}>
                <Typography className={classes.location} variant="h5">
                    {location}
                </Typography>
                {flagIcon && <img alt={contryCode} src={flagIcon} />}
            </div>
